...
export default function SignupPage() {
  ...
  // Password strength score and display properties
  const passwordStrength = React.useMemo(() => {
    if (!password) return { label: "", score: 0, color: "bg-gray-700" }
    let score = 0
    ...
    return { label: "Strong", score: 100, color: "bg-[#D3FF33]" }
  }, [password])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const signupResult = signup(trimmedName, trimmedEmail, password)

    if (signupResult.success) {
      // Auto login user after successful signup
      const loginResult = login(trimmedEmail, password)
      ...
      setIsSubmitting(false)
      setValidationError(signupResult.error || "Signup failed.")
        ...
        >
          ...
          <div className="space-y-1 text-left">
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400">
              Register now to start managing your energy metrics.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-3.5">
              ...
              {password && (
                <div className="pt-1.5 px-0.5 space-y-1">
                  <div className="flex items-center justify-between text-[9px] font-bold text-gray-400">
                    ...
                    <span className={passwordStrength.label === "Strong" ? "text-[#D3FF33]" : ""}>{passwordStrength.label}</span>
                  </div>
            ...
            <button
              type="submit"
              disabled={isSubmitting}
              className="w-full mt-3 bg-[#D3FF33] text-black hover:bg-[#b8e62c] hover:scale-[1.01] active:scale-[0.99] rounded-xl py-3.5 text-sm font-extrabold shadow-lg shadow-[#D3FF33]/10 transition-all duration-200 flex items-center justify-center gap-2 group disabled:opacity-50 disabled:cursor-not-allowed disabled:scale-100 disabled:hover:bg-[#D3FF33] cursor-pointer"
            >
              {isSubmitting ? (
